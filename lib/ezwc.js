...
class EzwcCore {
  ...
  /* istanbul ignore next */
  processFile(inFile, inFilePath, outFilePath) {
    Logger.process(`Transpiling %s...`, 'Start', inFile);

    const outFile = this.determineOutfile(inFile, inFilePath, outFilePath);
  ...
  process(inFilePath, outFilePath, watchForChanges, silenceLogger = false) {
    if (silenceLogger) {
