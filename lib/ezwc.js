...
class EzwcCore {
  ...
  processFile(inFilePath, outFilePath) {
    ...
    const styles = EzwcStyles.parseStyles($('style'), inFilePath);
    const $template = $('template');
    const template = EzwcTemplates.parseTemplate($template, styles, inFilePath);
    const script = EzwcScripts.parseScript($('script'), template, $template.attr('lang'), inFilePath);
  ...
  process(inFilePath, outFilePath, watchForChanges) {
    ...
    if (!watchForChanges) {
      ...
      watch(inFilePath, { recursive: true, filter: /\.ezwc$/ }, (event, filename) => {
        Logger.app('Changes found for %s', filename);
        this.processFile(filename, outFilePath);
      });
    }
  }
}
