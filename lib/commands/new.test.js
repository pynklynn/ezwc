...
describe('New command tests', () => {
  ...
  describe('initNpmProject tests', () => {
    ...
    test('should initialize NPM project with router and pug', () => {
      const tsBasePackageInit = Object.assign({}, basePackageInit);
      tsBasePackageInit.devDependencies.push('pug');
      EzwcNew.initNpmProject({ projectName: 'test-project', router: true, t: 'pug' });
      expect(createPackageJson).toHaveBeenCalledWith(tsBasePackageInit);
    });
...
importScript: true
  ...
  describe('deteremineEzwcTemplateExtension tests', () => {
    ...
    test('should determine the extension for pug', () => {
      const ext = EzwcNew.deteremineEzwcTemplateExtension({ t: 'pug', imports: [ 'template' ] });
      expect(ext).toBe('.pug');
    });
  });

  describe('getEzwcViewFileTemplate tests', () => {
    test('should generate the view template for pug', () => {
      const generatedTemplate = EzwcNew.getEzwcViewFileTemplate({ t: 'pug', projectName: 'pug' });
...
`
    ...
    test('should generate the view template for non-pug', () => {
      const generatedTemplate = EzwcNew.getEzwcViewFileTemplate({ t: 'html', projectName: 'non-pug' });
      expect(generatedTemplate).toBe(
        `
  <h1>Welcome to non-pug!</h1>
...
`
  ...
  describe('getEzwcComponentFileTemplate tests', () => {
    test('should generate the view template for pug', () => {
      const generatedTemplate = EzwcNew.getEzwcComponentFileTemplate({ t: 'pug', projectName: 'pug' });
      expect(generatedTemplate).toBe(
        `  h2 pug component
`
      );
    ...
    test('should generate the view template for non-pug', () => {
      const generatedTemplate = EzwcNew.getEzwcComponentFileTemplate({ t: 'html', projectName: 'non-pug' });
      expect(generatedTemplate).toBe(
        `
  <h2>non-pug component</h2>
`
      );
    });
  });
