  ...
  writeStylesheet(filename, outputFile, flags) {
    ...
    if (flags.importAll || flags.importStyles) {
      try {
        ...
        fs.writeFileSync(stylesFile, `/* replace with styles to this file for ${filename} */\n`, {
          encoding: 'utf8',
          flag: 'w+'
        });
      } catch(err) {
  ...
  writeTemplate(filename, outputFile, flags) {
    ...
    if (flags.importAll || flags.importTemplate) {
      try {
        ...
        Logger.error('Error writing out template file.');
        throw err;
      }
    }
  }
...
`,
        ...
        Logger.error('Error writing out script file.');
        throw err;
      }
    }
  }
