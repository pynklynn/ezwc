...
class EzwcScripts {
  ...
  generateScriptTag(selector, className, useTs, importScript) {
    const lang = useTs ? 'lang="ts" ' : '';
    const importAttr = importScript ? `src="./${selector}.ezwc.${useTs ? 'ts' : 'js'}" ` : '';
    const code = importScript ? '' : `
...
`;
    ...
    return `
<script ${lang}${importAttr}selector="${selector}">
${code}
</script>
`;
  }
